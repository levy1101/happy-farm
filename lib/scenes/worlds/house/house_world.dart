...
class HouseWorld extends World
  ...
  playerPositionAfterEnterHouse(){
    if(game.prevRoute != null && game.prevRoute == "farm"){
      player.direction = PlayerDirection.up;
      player.position = Vector2(640, 760);
    }
  }
  ...
  loadInteractCollision()async{
    await loadDoorInteractCollision();
  }

  loadDoorInteractCollision()async{
    final objectLayer = mapTiled.tileMap.getLayer<ObjectGroup>('door')!;
    for(final TiledObject object in objectLayer.objects){
      final block = IntersectionBlock(
        name: InteractName.houseDoorToFarm,
        position: Vector2(object.x * WORLD_SCALE, object.y * WORLD_SCALE),
        size: Vector2(object.width * WORLD_SCALE, object.height * WORLD_SCALE),
      );
      await add(block);
