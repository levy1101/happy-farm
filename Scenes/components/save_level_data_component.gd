...
var game_data_resource:SaveGameDataResource # level，export

func _ready() -> void:
	add_to_group("save_level_data_component") # ，save_game_manager
	level_scene_name = get_parent().name # （）
	
func save_node_data() -> void:
	var nodes = get_tree().get_nodes_in_group("save_data_component") # save_data_component
	
	game_data_resource = SaveGameDataResource.new() # newlevel
	
	if nodes != null:# save_data_component
...
func load_game() -> void:
	...
	var root_node: Window = get_tree().root # node
